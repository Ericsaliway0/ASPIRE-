...
def train(hyperparams=None, data_path='reactome_embedding/data/emb', plot=True):
    ...
    neo4j_uri = "neo4j+s://cea2223b.databases.neo4j.io"
    neo4j_user = "neo4j"
    neo4j_password = "OKbUZz48DTS7d8i-ra4qY9Ci544q-obaBfiB7vpeXd4"
...
def save_genes_in_pathways_to_csv(json_file, results_path, out_feats, num_layers, num_epochs):
    """
    Saves unique genes associated with pathways from a JSON file to a CSV file.
    
    Parameters:
    json_file (str): Path to the JSON file containing significant pathways.
    results_path (str): Directory where the CSV file will be saved.
    ...
    with open(json_file, 'r') as f:
        significant_pathways = json.load(f)
    
    # Convert to a DataFrame
    pathway_gene_data = []
    for cluster_id, pathways in significant_pathways.items():
        for pathway, genes in pathways.items():
            for gene in genes:
                pathway_gene_data.append([cluster_id, pathway, gene])
    
    df = pd.DataFrame(pathway_gene_data, columns=['Cluster ID', 'Pathway', 'Gene'])
    
    # Save to CSV
    csv_output_path = os.path.join(results_path, f'significant_genes_in_pathways_dim{out_feats}_lay{num_layers}_epo{num_epochs}.csv')
    df.to_csv(csv_output_path, index=False)
    
    print(f"Saved unique genes in pathways to: {csv_output_path}")
